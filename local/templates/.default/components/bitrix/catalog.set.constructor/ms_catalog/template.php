...
$blockWidth = 82/(1+$countDefSetItems);
?>
<div class="bx_item_set_hor_container_big">
	...
	<div class="bx_item_set_hor">
		<div class="bx_item_set_hor_item plus" style="width:<?=$blockWidth?>%;" data-price="<?=$arResult["ELEMENT"]["PRICE_DISCOUNT_VALUE"]?>" data-old-price="<?=$arResult["ELEMENT"]["PRICE_VALUE"]?>" data-discount-diff-price="<?=$arResult["ELEMENT"]["PRICE_DISCOUNT_DIFFERENCE_VALUE"]?>">
			...
			<a class="bx_item_set_linkitem" href="<?=$arResult["ELEMENT"]["DETAIL_PAGE_URL"]?>"><?=$arResult["ELEMENT"]["NAME"]?></a>
			<span class="bx_item_set_price">
				<?=$arResult["ELEMENT"]["PRICE_PRINT_DISCOUNT_VALUE"]?>
				<?if (!($arResult["ELEMENT"]["PRICE_VALUE"] == $arResult["ELEMENT"]["PRICE_DISCOUNT_VALUE"])):?><span><?=$arResult["ELEMENT"]["PRICE_PRINT_VALUE"]?></span><?endif?>
			</span>
		...
		<?foreach($arResult["SET_ITEMS"]["DEFAULT"] as $key => $arItem):?>
			<div class="bx_item_set_hor_item <?if ($key<$countDefSetItems-1) echo "plus"; else echo "equally"?> bx_default_set_items"
				...
				<a class="bx_item_set_linkitem" href="<?=$arItem["DETAIL_PAGE_URL"]?>"><?=$arItem["NAME"]?></a>
				
				
				<span class="bx_item_set_price">
					<?=$arItem["PRICE_PRINT_DISCOUNT_VALUE"]?>
					<span  class="bx_item_set_price old" <?if ($arItem["PRICE_VALUE"] == $arItem["PRICE_DISCOUNT_VALUE"]):?>style="display:none"<?endif?>><?=$arItem["PRICE_PRINT_VALUE"]?></span>
				</span>
		...
		<div class="bx_item_set_hor_item result">
			...
			<a href="javascript:void(0)" onclick="catalogSetDefaultObj_<? echo $intElementID; ?>.Add2Basket();" class="bt1 bt-small"><?=GetMessage("CATALOG_SET_BUY")?></a>
		</div>

		<div style="clear: both;"></div>
	</div>
	<a class="bx_item_set_creator_link dotted" href="javascript:void(0)" onclick="OpenCatalogSetPopup('<?=$intElementID?>');"><?=GetMessage("CATALOG_SET_CONSTRUCT")?></a>
</div>
				...
				onAfterPopupShow: function()
				{
					BX.ajax.post(
						...
						BX.delegate(function(result)
						{
							...
							var popupTop = $(document).scrollTop() + (window.innerHeight - BX("CatalogSetConstructor_"+element_id).offsetHeight)/2;
