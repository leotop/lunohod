...
if (!empty($arResult['ITEMS']))
{
	...
	{        
        ...
        while($ar_group = $db_old_groups->Fetch()){
            if($ar_group['IBLOCK_SECTION_ID'] == SERIES_SECTION_ID){
                $arResult['GROUPS_ID_LINKING'][$arItem["ID"]]["ID"] = $ar_group['CODE'];
                $arResult['GROUPS_ID_LINKING'][$arItem["ID"]]["SERIES_NAME"] = $ar_group['NAME'];    
            } 
        }
...
foreach($arResult['ITEMS'] as $ID => $arItems){                    
    $arWaterMark = Array(
        array(
            ...
            "type" => "image",
            "size" => "real",
            "file" => $_SERVER["DOCUMENT_ROOT"].'/upload/logoSmall.png', // Путь к картинке
            "fill" => "exact",
        )
    );
    ...
    $arResult['ITEMS'][$ID]["PREVIEW_PICTURE"]["SRC"] = $arFileTmp['src'];
    $arResult['ITEMS'][$ID]["PREVIEW_PICTURE"]["WIDTH"] = $arFileTmp['width'];
    $arResult['ITEMS'][$ID]["PREVIEW_PICTURE"]["HEIGHT"] = $arFileTmp['height'];
    $arResult['ITEMS'][$ID]['DISPLAY_PROPERTIES']['CATALOG_WEIGHT']['NAME'] = GetMessage('CATALOG_WEIGHT');
    $arResult['ITEMS'][$ID]['DISPLAY_PROPERTIES']['CATALOG_WEIGHT']['DISPLAY_VALUE'] = $arResult['ITEMS'][$ID]["CATALOG_WEIGHT"].' г';

    $arResult['ITEMS'][$ID]['DISPLAY_PROPERTIES']['CATALOG_SIZE']['NAME'] = GetMessage('CATALOG_SIZE');
    $arResult['ITEMS'][$ID]['DISPLAY_PROPERTIES']['CATALOG_SIZE']['DISPLAY_VALUE'] = $arResult['ITEMS'][$ID]["CATALOG_LENGTH"].'x'.$arResult['ITEMS'][$ID]["CATALOG_WIDTH"].'x'.$arResult['ITEMS'][$ID]["CATALOG_HEIGHT"].' мм';
}
