...
while($ar_group = $db_old_groups->Fetch()){
    if($ar_group['IBLOCK_SECTION_ID'] == SERIES_SECTION_ID){
        $arResult['GROUPS_ID_LINKING']["ID"] = $ar_group['CODE'];
        $arResult['GROUPS_ID_LINKING']["SERIES_NAME"] = $ar_group['NAME'];    
    } 
}
foreach($arResult['MORE_PHOTO'] as $ID => $WatermarkPhoto){
    $arWaterMark = Array(
        array(
            "name" => "watermark",
            "position" => "bottomcenter", // РџРѕР»РѕР¶РµРЅРёРµ
            "type" => "image",
            "size" => "real",
            "file" => $_SERVER["DOCUMENT_ROOT"].'/upload/logo2.png', // РџСѓС‚СЊ Рє РєР°СЂС‚РёРЅРєРµ
            "fill" => "exact",
        )
    );
    ...
    $arResult['MORE_PHOTO'][$ID]["SRC"] = $arFileTmp['src'];
    $arResult['MORE_PHOTO'][$ID]["WIDTH"] = $arFileTmp['width'];
    $arResult['MORE_PHOTO'][$ID]["HEIGHT"] = $arFileTmp['height'];
}
$arResult['DISPLAY_PROPERTIES']['CATALOG_WEIGHT']['NAME'] = GetMessage('CATALOG_WEIGHT');
$arResult['DISPLAY_PROPERTIES']['CATALOG_WEIGHT']['DISPLAY_VALUE'] = $arResult["CATALOG_WEIGHT"].' г';

$arResult['DISPLAY_PROPERTIES']['CATALOG_SIZE']['NAME'] = GetMessage('CATALOG_SIZE');
$arResult['DISPLAY_PROPERTIES']['CATALOG_SIZE']['DISPLAY_VALUE'] = $arResult["CATALOG_LENGTH"].'x'.$arResult["CATALOG_WIDTH"].'x'.$arResult["CATALOG_HEIGHT"].' мм';
?>